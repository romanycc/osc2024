 ...
 * If timeout, run the callback function, which print character.
 ****************************************************************/
...
int enable_timer_int(void) {
  asm volatile("mrs	x0, 	cntfrq_el0;"  // get freqency
               "mov	x1, 	100;"
               "mul	x0, 	x0, x1;"
               "msr	cntp_tval_el0, x0;" // set large expire time
               "mov	x0, 	2;"
               "ldr	x1, 	=0x40000040;" // Timer interrupt,CORE0_TIMER_IRQ_CTRL
               "str	w0,	[x1];");
  return 0;
}
