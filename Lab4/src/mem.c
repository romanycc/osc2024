...
static unsigned int sarr[5];
static unsigned int countarr[5];
...
static int gen_slots(int size) {
  ...
  // uart_puti(t);
  // uart_puts("\n");
  char *mem = (uint32_t *)pmalloc(0);
  sarr[size] = mem;
  for (int i = 0; i < FRAME_SIZE; i += t*4) {
    list_adds(size, (void *)(mem + i));
    // if (i<10 || i>4050){
    //   uart_puth((void *)(mem + i));
    //   uart_puts("\n");
    // }
  }
  // Log
...
void *smalloc(int size) {
  ...
  uart_puti(size);
  uart_puts("* 4 bytes\n");
  if (free_smem[size-1]==NULL){
    gen_slots(size-1); 
  }
  return list_pops(size-1);
}


// void* sfree(void* addr){
//         return list_delete(addr);
// }
int sfree(void *addr) { 
  int cnt = -1;
  for (int i=0;i<SMEM_MAX;i++){
    // uart_puts("++++++++");
    // uart_puts("\n");
    // uart_puth((int)addr & 0xFFFFF000);
    // uart_puts("\n");
    // uart_puth( sarr[i] & 0xFFFFF000);
    // uart_puts("\n");
    if (((int)addr & 0xFFFFF000) == ((int)sarr[i] & 0xFFFFF000)){
      // uart_puts("here\n");
      cnt = i;
      break;
    }
  ...
  for (int i=0;i<SMEM_MAX;i++){
    ...
    // uart_puts("\n");
    // if (i==0){
    //   uart_puti(countarr[i]);
    //   uart_puts("\n");
    // }
    // uart_puti(0x1000/(4*tmp)-1);    
    // uart_puts("\n");
    if(countarr[i] == (unsigned int)(0x1000/(4*tmp)-1)){
      uart_puts("Free to pmalloc!\n");
      uart_puth(sarr[i]);
      uart_puts("\n");
      pfree(sarr[i] & 0xFFFFF000);
    }
    tmp = tmp * 2; 
  }
}
